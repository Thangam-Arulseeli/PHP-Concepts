...
if(!empty($_POST['update']) && !empty($_POST['id']) )  {
  ...
  $query2 = "SELECT * FROM userdata WHERE id='".$_POST['id']."'";
...
//   $id=$_GET['id'];
//   // Fetch record with id and populate it in the form
//   $query2 = "SELECT * FROM userdata WHERE id='".$_GET['id']."' ";
//   $result = $conn->query($query2);
//   if ($result->num_rows > 0) {
//     // Output data for each row
//     while($row = $result->fetch_assoc()) {
...
if(!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['id']) ){
  ...
  $query = "UPDATE userdata SET name='".$_POST['name']."', email='".$_POST['email']."' WHERE id='".$_POST['id']."' ";
  if (mysqli_query($conn, $query)) {
      echo "Record updated successfully!<br/>";
      echo '<a href="form-post.php"> BACK </a>';
    //   '<a href="form-get.php">Get Form</a><br/>'
      die(0);
  } else {
...
<body>
    ...
    <form method="POST" action="update.php">
        ID: <input type="text" name="id" value="<?php echo($id); ?>" hidden><br><br/>
